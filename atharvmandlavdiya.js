...
(function() {
  ...
  var svgBlob = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + svgW + ' ' + svgH + '" width="' + rW + '" height="' + rH + '">'
    + '<path d="${PATH_D}" fill="none" stroke="white" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>'
    + '</svg>';
...
if (typeof customElements !== 'undefined' && !customElements.get('dtc-logo')) {
  customElements.define('dtc-logo', class extends HTMLElement {
    connectedCallback() {
      const doc = new DOMParser().parseFromString(dtcLogo(), 'text/html');
      const canvas = doc.querySelector('canvas');
      if (canvas) this.appendChild(canvas);
      const s = document.createElement('script');
      s.textContent = doc.querySelector('script').textContent;
